...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_claude_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same as the CLI handler in cli.py:
            /claude-runtime              show current state
            /claude-runtime auto         Hermes default runtime
            /claude-runtime claude_agent Claude Code runtime
            /claude-runtime on / off     synonyms

        On change, the cached agent for this session is dropped so the next
        message builds a fresh AIAgent with the new api_mode."""
